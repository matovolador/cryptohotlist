...
def defineMYSQL():
	db = MySQLdb.connect(host="localhost", user="root", passwd="secret", db="kraken_bot", cursorclass=MySQLdb.cursors.DictCursor)       
	return db
def defineMYSQLCash():
...
def insertData():
	...
	if (data!=False):
		...
		cur.close()
		print("Could not get data from Kraken.")
...
def getEURPrice():
	db = defineMYSQLCash()
	cur = db.cursor()
	cur.execute("SELECT * FROM pairs WHERE symbol=%s ORDER BY id DESC LIMIT 1",['EUR'])
	EUR = cur.fetchone()
	cur.close()
	if EUR:
