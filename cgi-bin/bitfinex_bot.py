...
def defineMYSQL():
	...
	db = MySQLdb.connect(host="localhost", user="root", passwd="secret", db="bitfinex_bot", cursorclass=MySQLdb.cursors.DictCursor)       
	return db
...
def getData():
	print("Getting data...")
	try:
		res = bitfinex.ticker('bchusd')
		obj = {}
		obj['name'] = 'Bitcoin Cash'
		obj['symbol'] = 'BCH'
		obj['volume'] = res['volume']
		obj['price'] = res['last_price']
	except Exception, e:
		print("Error getting JSON from bitfinex: "+str(e))
		return False
	return obj
...
def getBTCPrice():
	...
	# get data from API
	try:
		response = requests.get('https://api.coindesk.com/v1/bpi/currentprice.json','GET')
		data = response.json()
		btcPrice = data['bpi']['USD']['rate']
		btcPrice = btcPrice.replace(',','')
		return float(btcPrice)
	except Exception, e:
		print("Error getting BTC price from Coindesk: "+str(e))
		return False		
