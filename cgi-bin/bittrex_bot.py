...
def defineMYSQL():
	#local:
	#db = MySQLdb.connect(host="localhost", user="root", passwd="secret", db="bittrex_bot", cursorclass=MySQLdb.cursors.DictCursor)       
	#server:
	db = MySQLdb.connect(host="localhost", user="matias", passwd="garfunkel_1548A", db="bittrex_bot", cursorclass=MySQLdb.cursors.DictCursor)       
	return db
...
def getData():
	print("Getting data...")
	try:
		res = requests.get('https://bittrex.com/api/v1.1/public/getmarketsummary?market=USDT-BCC','GET')
		res = res.json()
		obj = {}
		obj['name'] = 'Bitcoin Cash'
		obj['symbol'] = 'BCH'
		obj['volume'] = str(res['result'][0]['Volume'])
		obj['price'] = str(res['result'][0]['Last'])
	except Exception, e:
		print("Error getting JSON from Bittrex: "+str(e))
		return False
