...
$symbols = $CHL->getTopRanks(100,1,100);
for ($h=0;$h<count($symbols);$h++){
	$symbol = $symbols[$h]['symbol'];
	$name = $symbols[$h]['name'];
	$res = $CHL->getRankChanges($name);
	?>
	...
	$(document).ready(function(){
		...
		for (i=0;i<res.length;i++){
			if (i==0){
				...
				if (res[i]['rank']<res[i-1]['rank']){
					amount = ""+res[i]['rank'];
					numberFlag = "+";
				}else if(res[i]['rank']>res[i-1]['rank']){
					amount = ""+res[i]['rank'];
					numberFlag = "-";
				}else{
					amount = ""+res[i]['rank'];
			...
			myData[i] = {rank : amount, numberFlag: numberFlag, time: res[i]['time_created']};	
		}
		...
		for (i=0;i<myData.length;i++){
			chartData[i] = myData[i]['rank'];
			if (myData[i]['numberFlag']=="-"){
				colorData[i] = "rgba(215, 0, 0,1)";
				labelData[i] = "Time: "+myData[i]['time']+" Rank";
			}else if (myData[i]['numberFlag']==""){
